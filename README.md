...
Примерно тоже самое, что и для верстальщика, т.к. для того, чтобы делать клиентскую часть, 
нужно понимать архитектуру вёрстки, а также интуитивно понятный сам код и грамотная архитектура (наименование сущностей, классов и ID
"+" архитектура сайта/ приложения с точки зрения производительности, т.к. производительность клиентской части напрямую зависит от 
производительности самой верстки (сколько ресурсов требует реализованный код)).
...
поддерживаемость проекта ( [Bootstrap](https://getbootstrap.com), [jquery](https://jquery.com), [npm](https://www.npmjs.com) и др.).
...
кросплатформенный чат на [Node.js](https://nodejs.org/en/), использовал [npm](https://www.npmjs.com) и др.) (для хранения списка всех использованных фреймворков в json и удобной разверстки на
др. ПК), верстку делал как на чистом HTML/CSS/JS (использовал принцип [флексбоксов](https://html5book.ru/css3-flexbox/) ```#container{display: flex;}``` ), так и [ejs](http://ejs.co) (шаблонизатор), [jquery](https://jquery.com),
серверную часть на [вебсокетах](http://websocket.org) .
...
(таковыми не пользуюсь, учу наизусть всё используемое в коде) до дебаггинга, современные фреймворки описанные выше (в основном знаю относительно поверхностно, нужно больше практики, для полного ознакомления).
...
для быстрой и гибкого дебаггинга сайта (вывести что-то в лог, наглядно и «на месте» пофиксить проблемы со стилями и вёрсткой, и работой 
...
> ### ***Q7	-	Какие ресурсы вы используете для развития в профессиональной сфере? Приведите несколько конкретных примеров (сайты, блоги и так далее).***
  ...
  Подписан на блоги в ВК, телеграмм по тематикам программирования, веб-разработки и front-end (публикации новинок, новостей), 
  <habr.com> , подписки на каналы ютуб, читаю книги, просто поиск в гугле, изучаю разные реализованные вещи на [github](github.com) и др).
